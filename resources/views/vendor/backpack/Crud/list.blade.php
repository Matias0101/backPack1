...
<div class="row" bp-section="crud-operation-list">
  ...
  <div class="{{ $crud->getListContentClass() }}">
    ...
    <div class="row mb-2 align-items-center">
      ...
      <!-- reemplazo codigo con cam para restringir -->
      {{-- Mostrar botón de crear usuario solo si el usuario tiene el permiso 'manage users' --}}
      @can('manage users')
    ...
    <div class="{{ backpack_theme_config('classes.tableWrapper') }}">
      ...
      @endphp
              <th data-orderable="{{ var_export($column['orderable'], true) }}" data-priority="{{ $column['priority'] }}"
                data-column-name="{{ $column['name'] }}" {{-- data-visible-in-table=> if developer forced column to be in
                the table with 'visibleInTable => true'
      ...
      @endforeach
        ...
        <th data-orderable="false" data-priority="{{ $crud->getActionsColumnPriority() }}"
          ...
          {{-- Verificación de permisos antes de mostrar el botón de acciones --}}
          @can('manage categories')
